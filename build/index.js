...
vorpal.command('createOrder <name> <phoneNumber>', 'Create an order and saves it as a JSON file').action(function (args, callback) {
  ...
  this.prompt({
    ...
    _this.prompt(questions, function (response) {
      var order = (0, _lib.updateOrderTotal)(_toConsumableArray(Array(Number.parseInt(numLemonades))).map((0, _lib.createLemonade)(response)).reduce(_lib.addLemonadeToOrder, {
        total: 0,
        lemonades: [],
        customer: {
          name: args.name,
